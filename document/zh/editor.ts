...
export const item: Type.Item = {
	...
	children: {
		...
		equipmentType: {
			...
			points: ["装備可能なアイテムは、カテゴリの`equipmentGroups`で指定されたグループに属している必要があります。", "同一组内只能装备一件物品。", "`unnecessary` 无论是否装备都会发挥效果，效果量与持有数成正比（持有 0 个时无效果，持有 n 个时为 n 倍）。"],
			list: [
				["值", "装备", "有效性。", "消费量", "例如"],
...
export const category: Type.Category = {
	...
	children: {
		...
		equipmentGroups: {
			...
			points: ["このカテゴリで装備できるアイテムのグループ名のリストです。", "只有与此处指定的组名称相匹配的组中的物品才能装备。", "如果指定的是空字符串，那么未配置组（空）的物品就有资格装备。", "同一组中只能装备一件物品。装备新物品时，同组中的任何现有装备都会自动卸下。", "如果列表为空（0 件），则无法装备任何物品。"],
			links: { general: "general" },
		},
		numeric: {
			title: "等级的可用性",
